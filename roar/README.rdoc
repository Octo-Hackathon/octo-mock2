== README
ROAR is Ruby on Rails application 
...
Create Project by using skip_active_record
...
make sure rake 10.4.2 is installed if not install rake by
gem intall  rake
bundle install
...
either local RAILS server or Apache. The path structure should like the one noted below
